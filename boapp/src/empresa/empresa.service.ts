  ...
  async getCotizacionesEmpresa(codEmpresa: string, fechaDesde: string, fechaHasta: string, escala: string): Promise<any[]> {
    ...
    if (escala === 'hora') {
      return this.cotizacionRepository
        ...
        .addSelect('SUBSTRING_INDEX(GROUP_CONCAT(cotizacion.cotization ORDER BY CONCAT(cotizacion.fecha, " ", cotizacion.hora) ASC), ",", 1)', 'apertura')
        .addSelect('SUBSTRING_INDEX(GROUP_CONCAT(cotizacion.cotization ORDER BY CONCAT(cotizacion.fecha, " ", cotizacion.hora) DESC), ",", 1)', 'cierre')
